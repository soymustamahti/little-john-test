        ...
        *,
        ...
        if not self._settings.is_configured:
            raise RerankerUnavailableError(
                "OpenAI API settings are incomplete for reranking."
            )

        prompt = _build_reranking_prompt(
        ...
        if parsed is None:
            raise RerankerUnavailableError(
                "OpenAI reranking returned no structured output."
            )

        candidates_by_index = {
            candidate.chunk_index: candidate for candidate in candidates
        }
        ...
        if not reranked:
            raise RerankerUnavailableError(
                "OpenAI reranking returned no valid chunk indexes."
            )

        return reranked


def build_document_reranker(
    settings: OpenAIProviderSettings,
