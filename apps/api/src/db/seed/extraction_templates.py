...
from src.extraction_templates.schemas import ExtractionTemplateCreate


class ExtractionTemplateSeed(ExtractionTemplateCreate):
    id: UUID

    def to_insert_values(self) -> dict[str, Any]:
        serialized_modules = self.model_dump(mode="json")["modules"]
        return {
            "id": self.id,
            "name": self.name,
...
RAW_EXTRACTION_TEMPLATE_SEEDS = [
    {
        "id": "11111111-1111-4111-8111-111111111111",
        "name": "Vendor Invoice",
        "description": (
            "Comprehensive invoice template for vendor bills, including payment terms "
    ...
    },
    {
        ...
        "name": "Purchase Order",
        "description": (
            "Procurement-ready purchase order template covering supplier, buyer, "
            "logistics, and ordered items."
        ),
        "locale": "en",
        "modules": [
            {
    ...
    },
    {
        ...
        "description": (
            "Long-form contract template for agreement metadata, dates, commercials, "
            ...
            },
            {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "required": False,
                        "description": (
                            "Cancellation notice period expressed in days."
                        ),
                        "kind": "scalar",
                        "value_type": "number",
                    },
            ...
            },
            {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "required": False,
                        "description": (
                            "Monthly, quarterly, yearly, or milestone-based."
                        ),
                        "kind": "scalar",
                        "value_type": "string",
                    },
                ...
                "fields": [
                    {
                        ...
                        "required": False,
                        "description": (
                            "Project milestones or acceptance checkpoints."
                        ),
                        "kind": "table",
                        "min_rows": 0,
                        "columns": [
                            ...
                            },
                            {
                                ...
                                "required": False,
                                "description": (
                                    "Commercial amount tied to the milestone."
                                ),
                            },
                            {
    ...
    },
    {
        ...
        "description": (
            "Modele francophone de facture fournisseur avec resume fiscal "
            "et lignes de facturation detaillees."
        ),
        "locale": "fr",
        "modules": [
            {
            ...
            },
            {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "required": False,
                        "description": (
                            "Modalites de paiement mentionnees sur la facture."
                        ),
                        "kind": "scalar",
                        "value_type": "string",
                    },
                ...
                "fields": [
                    {
                        ...
                        "required": False,
                        "description": (
                            "Prestations ou produits detailles sur la facture."
                        ),
                        "kind": "table",
                        "min_rows": 1,
                        "columns": [
            ...
            },
            {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "required": False,
                        "description": ("Personne habilitee a engager le mandataire."),
                        "kind": "scalar",
                        "value_type": "string",
                    },
                ],
            },
            {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "required": False,
                        "description": ("Montant total hors taxes de l'operation."),
                        "kind": "scalar",
                        "value_type": "number",
                    },
                    {
                        ...
                        "columns": [
                            {
                                ...
                                "value_type": "string",
                                "required": True,
                                "description": ("Numero ou identifiant de l'annexe."),
                            },
                            {
                                "key": "intitule_annexe",
...
EXTRACTION_TEMPLATE_SEEDS = [
    ExtractionTemplateSeed.model_validate(seed)
    for seed in RAW_EXTRACTION_TEMPLATE_SEEDS
]


def get_missing_extraction_template_seeds(
    existing_ids: set[UUID],
    existing_names: set[str],
) -> list[ExtractionTemplateSeed]:
    return [
        seed
        for seed in EXTRACTION_TEMPLATE_SEEDS
        if seed.id not in existing_ids and seed.name not in existing_names
    ]


async def seed_extraction_templates(session: AsyncSession) -> int:
    existing_ids_result = await session.execute(select(ExtractionTemplateModel.id))
    existing_names_result = await session.execute(select(ExtractionTemplateModel.name))
    existing_ids = set(existing_ids_result.scalars().all())
    existing_names = set(existing_names_result.scalars().all())
    missing_seeds = get_missing_extraction_template_seeds(existing_ids, existing_names)

    if not missing_seeds:
        return 0

    insert_statement = postgresql.insert(ExtractionTemplateModel).values(
        [seed.to_insert_values() for seed in missing_seeds]
    )
    await session.execute(
        insert_statement.on_conflict_do_nothing(index_elements=["id"])
    )
